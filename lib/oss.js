...
OSS.prototype.getHeaders = function (headers, options) {
	headers = headers || {}
	...
	headers['Authorization'] = this.generateSign(options.method, headers, resource)
...
OSS.prototype.request = function (method, headers, options, callback) {
	...
	headers  = self.getHeaders(headers, options)
	...
	if (options.source) {
		if (options.source instanceof Stream) {
			...
			fs.createReadStream(options.source).pipe(req)
...
OSS.prototype.listBucket = function (callback) {
	...
	this.request('GET', null, null, callback)
}
